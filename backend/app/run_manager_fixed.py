...
from app.orchestration_pipeline import run_agent_orchestration
...
def event_to_websocket_payload(event: models.Event) -> dict:
    ...
    payload.update(json.loads(event.metadata_json) if event.metadata_json else {})
    return payload
...
async def run_agent_timeline(db: Session, websocket: WebSocket, task: str) -> str:
    """
    Create a run, stream agent timeline events, and return the new run id.
    This replaces the fake timeline with real agent logic.
    """
    ...
    if cached_decision:
        ...
        orchestration_success = await run_agent_orchestration(db, websocket, run.id, task)
        
        if not orchestration_success:
            adapter_failed = True
            run_allowed = False
        else:
            # Continue with security evaluation if orchestration succeeded
